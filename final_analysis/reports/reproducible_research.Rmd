...
* Different types of observational units get different data tables
...
In addition to documenting the data, the code itself must be documented. Code should be well commented and written in a manner that is conducive to human reading. Writing readable code makes code easier to understand, easier to debug and improve, and makes collaboration a smoother process. A reader should be able to open a script without any prior knowledge of it and in a reasonably short amount of time have a general understanding of what the code does and how it does it.
...
Finally, all of the above components need to be organized and accessible. Ideally, any individual wishing to reproduce the results of a particular project should be able to quickly find that project in an archive or repository and be able to identify the purpose and location of all the major project components.

When dealing with large projects, file-clutter can quickly become overwhelming and significantly reduce productivity. One way to avoid this is to start projects with consistent, well-organized directory structures. Consistency allows your future self and other researchers to quickly identify where particular files are stored based on subject. Finding the right files is a critical step in reproducing research.
...
* Consistent file organization: Directory and R Package structure
